...
def situation_for_llm(session: AgentSession) -> dict[str, Any]:
    ...
    return {
        ...
        "note": (
            ...
            "They are not P(this pixel burns). ETA and P(burn by T) come from ELMFIRE. "
...
def handle_watch_note(session: AgentSession, args: dict[str, Any]) -> dict[str, Any]:
    ...
    steps = [
        "Keep the watch cadence; do not treat this as an order to evacuate.",
    ...
    if sample.get("eta_hours") is None and session.spread_field:
        steps.insert(0, "Engine field exists but this site is outside a reached cell (ETA null, not zero).")
...
def handle_prepare_plan(session: AgentSession, args: dict[str, Any]) -> dict[str, Any]:
    steps = [
        "Stage documents and a site contact list. This is not an evacuate order.",
...
def handle_suppression_steps(session: AgentSession, args: dict[str, Any]) -> dict[str, Any]:
    ...
    steps = [
        f"ETA {sample.get('eta_hours')} h (engine). Protect the asset; this is not an official perimeter.",
        f"Contact {_agency(session)}.",
        f"Access: nearest_road_class={_road(session)}.",
        f"Water mention (Mireye): {water or 'none fetched — do not invent a hydrant.'}",
        "Clear defensible space only if staff can do so before arrival; do not invent hose lays.",
    ]
    session.playbook_steps = steps
...
def handle_evac_steps(session: AgentSession, args: dict[str, Any]) -> dict[str, Any]:
    steps = [
        ...
        "Use the response dossier routes; do not invent a second exit.",
    ]
    session.playbook_steps = steps
...
def handle_inspect(session: AgentSession, args: dict[str, Any]) -> dict[str, Any]:
    steps = [
        ...
        "Do not keep an evacuate or protect order active.",
    ]
    session.playbook_steps = steps
